...
# ╔═════════════════════════════════════════════════════════════════╗
# ║ ⚙️ /setup                                                       ║
# ║ Command to configure server calendars and user mappings         ║
# ╚═════════════════════════════════════════════════════════════════╝
@bot.tree.command(
    name="setup",
    description="Configure calendars for the server"
)
@app_commands.describe(
    action="Action to perform: add, remove, list",
    calendar_url="Google Calendar ID or ICS URL",
    user="User to associate with this calendar"
)
async def setup_command(
    ...
    try:
        ...
        # Different actions based on the subcommand
        if action.lower() == "add":
            if not calendar_url:
                await interaction.response.send_message(
                    "⚠️ Please provide a calendar URL or ID.", 
                    ephemeral=True
                )
                return
            ...
            if not user:
                await interaction.response.send_message(
                    "⚠️ Please specify which user to associate with this calendar.", 
                    ephemeral=True
                )
                return
                
            # Defer response since we might need to make API calls
            await interaction.response.defer(ephemeral=True)
            
            # Add the calendar
            display_name = f"{user.display_name}'s Calendar"
            ...
            # Add instructions for Google Calendar sharing if it's a Google calendar
            if success and "google" in calendar_url.lower():
                service_email = get_service_account_email()
                message += f"\n\n**Important**: Share your Google Calendar with this service account:\n`{service_email}`\n\nGive it **Make changes to events** permission."
            ...
            if not calendar_url:
                await interaction.response.send_message(
                    "⚠️ Please provide the calendar URL or ID to remove.", 
                    ephemeral=True
                )
                return
            ...
            # Remove the calendar
            success, message = remove_calendar(interaction.guild_id, calendar_url)
            ...
            if not calendars:
                await interaction.followup.send(
                    "No calendars configured for this server yet. Use `/setup add` to add calendars.", 
                    ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                "⚠️ Invalid action. Valid options are: `add`, `remove`, `list`.", 
                ephemeral=True
            )
        ...
        await interaction.followup.send(
            f"An error occurred while configuring calendars: {str(e)}", 
            ephemeral=True
        )
