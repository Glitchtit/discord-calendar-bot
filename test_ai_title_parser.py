...
def test_swedish_math_course():
    ...
    # Use safe slicing to avoid IndexError
    first_two_words = ' '.join(result_words[:min(2, len(result_words))]).lower()
    ...
    first_three_words = ' '.join(result_words[:min(3, len(result_words))]).lower()
