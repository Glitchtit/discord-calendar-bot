...
from log import logger  # Unified logger
...
def generate_greeting(event_titles: list[str]) -> tuple[str | None, str]:
    try:
        ...
        logger.debug("Calling OpenAI for greeting...")
...
def generate_image(greeting: str, persona: str, max_retries: int = 3) -> str | None:
    ...
    for attempt in range(max_retries):
        try:
            logger.debug(f"Attempt {attempt + 1}: generating image for {persona}")
            ...
            if hasattr(e, "status_code") and e.status_code == 400 and "content_policy_violation" in str(e).lower():
                logger.warning(f"Content policy violation on attempt {attempt + 1}")
                ...
                logger.exception("Unexpected error on image generation")
                if attempt + 1 == max_retries:
                    logger.error("Max retries hit. Skipping image.")
                    return None
