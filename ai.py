...
def generate_greeting(event_titles: list[str]) -> tuple[str | None, str]:
    try:
        ...
        log.debug(f"Generating greeting for events: {event_summary}")

        style = random.choice(["butler", "bard", "alchemist", "decree"])
        log.debug(f"Selected persona style: {style}")
        ...
        if style == "butler":
            ...
            system_msg = "Thou art a deeply loyal medieval butler who speaketh in reverent, formal Elizabethan English. " + base_instruction
            ...
            system_msg = "Thou art a poetic bard, who doth speak in rhymes and jests and singsongs of yore. " + base_instruction
            ...
            system_msg = "Thou art an eccentric and prophetic alchemist, rambling in visions and olde-tongue riddles. " + base_instruction
            ...
            system_msg = "Thou art the herald of the crown, proclaiming stately decrees in archaic, noble tongue. " + base_instruction

        log.debug("Calling OpenAI for greeting...")
        ...
        message += f"\n\n— {persona}"
        log.info(f"Greeting generated by {persona}")
        return message, persona
    except Exception as e:
        log.exception("Failed to generate greeting")
...
def generate_image(greeting: str, persona: str, max_retries: int = 3) -> str | None:
    ...
    for attempt in range(max_retries):
        try:
            log.debug(f"Attempt {attempt + 1}: generating image for {persona}")
            ...
            if hasattr(e, "status_code") and e.status_code == 400 and "content_policy_violation" in str(e).lower():
                log.warning(f"Content policy violation on attempt {attempt + 1}")
                if attempt + 1 < max_retries:
                    time.sleep(1)
                    continue
                else:
                    log.error("Image prompt blocked after multiple attempts.")
                    return None
            else:
                log.exception("Unexpected error on image generation")
                if attempt + 1 == max_retries:
                    log.error("Max retries hit. Skipping image.")
                    return None
...
def post_greeting_to_discord(events: list[dict] = []):
    ...
    try:
        ...
        if resp.status_code not in [200, 204]:
            ...
            log.info("Discord greeting post successful.")
    except Exception as e:
        log.exception("Failed to post greeting to Discord.")
